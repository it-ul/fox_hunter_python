...
RULES_TEXT = (
    ...
    "Как играть:\n"
    "• ЛКМ (левая кнопка мыши) по клетке открывает её и показывает пеленг — "
    "сколько лис стоит на той же вертикали, горизонтали и обеих диагоналях.\n"
    "• Пеленг 0 означает, что на всех линиях клетки лис нет — такие линии "
    "закрашиваются серым.\n"
    "• ПКМ (правая кнопка мыши) помечает клетку серым (вы считаете, что там "
    "пусто); повторный ПКМ снимает пометку. Помеченную клетку всё равно можно "
    "открыть ЛКМ.\n"
    ...
    "• Кнопка «Сохранить» записывает текущую игру (поле, пометки и протокол) "
    "в JSON-файл; «Загрузить» открывает сохранённую игру и позволяет продолжить "
    "её с того же места.\n\n"
